...
public class Graph<T,U extends Number & Comparable> {
    ...
    public Graph<T,U> truncate(U min, U max) {
        ...
        List<Arc<T,U>> truncArcs =
                arcs.stream().filter(arc ->
                        min.compareTo(arc.weight) < 0 && max.compareTo(arc.weight) > 0
    ...
    public Graph<T,U> routeTo(T nodeFrom, T nodeTo) {
        if(!nodes.contains(nodeFrom) || !nodes.contains(nodeTo)) {
            throw new IllegalArgumentException("One or both of the provided nodes do not exist in the graph.");
        }
    ...
    private Graph<T,U> dijkstra(T nodeFrom, T nodeTo) {
        ...
        nodes.stream().filter(n -> !n.equals(nodeFrom)).forEach(n -> distances.put(n, Double.POSITIVE_INFINITY));
        ...
        while(unvisitedNodes.contains(nodeTo)) {
            ...
            List<Arc<T,U>> neighbours = arcs.stream().filter(arc -> arc.connects(finalCurrentNode)).collect(Collectors.toList());
